...
import me.travis.wurstplusthree.hack.hacks.render.ItemPhysics;
import me.travis.wurstplusthree.mixin.mixins.MixinRenderer;
            ...
            if ((itemstack = (entityitem = entity).getItem()).getItem() != null) {
                ...
                for (int j = 0; j < i; ++j) {
                    if (ibakedmodel.isAmbientOcclusion()) {
                        GlStateManager.pushMatrix();
                        GlStateManager.scale((double)ItemPhysics.INSTANCE.Scaling.getValue(), (double)ItemPhysics.INSTANCE.Scaling.getValue(), (double)ItemPhysics.INSTANCE.Scaling.getValue());
